...
class Tool(ABC):
    ...
    def get_parameters(self) -> List[ToolParameter]:
        ...
        Returns:
            List[ToolParameter]: 工具参数定义列表
        """
        pass

    def get_paramteters(self) -> List[ToolParameter]:
    ...
    def run_with_timing(self, parameters: Dict[str, Any]) -> ToolResponse:
        """执行工具，并且添加工具执行时间统计和上下文信息

        Args:
            paramters (Dict[str,Any]): 工具参数

        Returns:
            ToolResponse: 包含工具执行时间统计的响应对象
        """
        start_time:float = time.time()
        
        try:
            response: ToolResponse = self.run(parameters)
        except Exception as e:
            elapsed_ms = int((time.time() - start_time) * 1000)
            return ToolResponse.error(
                code=ToolErrorCode.INTERNAL_ERROR,
                message= f"工具执行时出现未处理的异常：{str(e)}",
        ...
        if not isinstance(response, ToolResponse):
            ...
            return ToolResponse.error(
                code=ToolErrorCode.INTERNAL_ERROR,
                message="Tool.run() must return a ToolResponse instance",
                stats={"time_ms": elapsed_ms},
                context={"params_input": parameters, "tool_name": self.name},
            )
    ...
    async def arun_with_timing(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        if not isinstance(response, ToolResponse):
            ...
            return ToolResponse.error(
                code=ToolErrorCode.INTERNAL_ERROR,
                message="Tool.arun() must return a ToolResponse instance",
                stats={"time_ms": elapsed_ms},
                context={"params_input": parameters, "tool_name": self.name},
            )
...
class AutoGeneratedTool(Tool):
    ...
    async def arun(self, parameters: Dict[str, Any]) -> ToolResponse:
        """Execute a generated action and await it when necessary."""
        try:
            ...
            return ToolResponse.error(
                ...
                message=f"Method execution failed: {e}",
            )
    ...
    def run(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        try:
            ...
            if inspect.isawaitable(result):
                ...
                return ToolResponse.error(
                    code=ToolErrorCode.EXECUTION_ERROR,
                    message="Async tool actions must be executed with arun()",
                )
